                                    ...
                                    <div className="absolute inset-x-0 -bottom-20 flex items-center justify-center">
                                        <div className="bg-emerald-500/10 backdrop-blur-xl border border-emerald-500/20 px-10 py-4 rounded-full">
                                            <p className="font-serif italic text-2xl text-emerald-400">
                                                Frame {selectedImg + 1} // {images.length}
                                            </p>
                                        </div>
                                    </div>
                                </motion.div>
...
const Navbar: FC<{ theme: "light" | "dark"; toggleTheme: () => void }> = ({ theme, toggleTheme }) => {
    ...
    return (
        <nav className={`fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-32 pointer-events-none'}`}>
            <div className="bg-[#0f0f11]/80 backdrop-blur-3xl border border-white/10 px-8 py-5 rounded-[2.5rem] shadow-2xl flex items-center gap-6 sm:gap-10">
                {navItems.map((item, idx) => (<a key={idx} href={item.href} className="group relative text-white/40 hover:text-emerald-400 transition-all"><item.icon size={22} /><span className="absolute -top-16 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all bg-emerald-500/20 backdrop-blur-xl border border-emerald-500/30 text-emerald-400 text-[10px] px-5 py-3 rounded-full hidden group-hover:block whitespace-nowrap">{item.label}</span></a>))}
                <button onClick={toggleTheme} className="text-white/40 hover:text-emerald-400 transition-all">{theme === "light" ? <Moon size={22} /> : <Sun size={22} />}</button>
            </div>
        </nav>
    );
};
