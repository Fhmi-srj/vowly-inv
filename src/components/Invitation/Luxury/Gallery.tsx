...
const Gallery: React.FC = () => {
  ...
  const navigate = (direction: "prev" | "next", e?: React.MouseEvent) => {
    ...
    if (direction === "prev") {
      setSelectedImg(
        selectedImg === 0 ? config.galleryImages.length - 1 : selectedImg - 1
      );
    } else {
      setSelectedImg(
        selectedImg === config.galleryImages.length - 1 ? 0 : selectedImg + 1
      );
    }
  };
  ...
  return (
    ...
    >
      <div className="container mx-auto max-w-7xl px-6">
        ...
        <div className="grid grid-cols-2 gap-6 md:gap-10 max-w-4xl mx-auto">
          {config.galleryImages.slice(0, 2).map((src, index) => (
            <div
              key={index}
            ...
            >
              <img
                ...
                className="w-full h-auto transform object-contain grayscale-[0.2] transition-transform duration-[1.5s] ease-out group-hover:scale-105 group-hover:grayscale-0"
                loading="lazy"
              />
      ...
      {selectedImg !== null && (
        <div
          className={`fixed inset-0 z-[1000] flex items-center justify-center overflow-hidden transition-all duration-500 ${isClosing ? "opacity-0" : "opacity-100"
            }`}
        >
          <div
          ...
          <div className="pointer-events-none relative z-[1010] flex h-full w-full items-center justify-center">
            <div className="relative flex max-h-[75vh] max-w-[95vw] items-center justify-center md:max-h-[85vh]">
              <img
                ...
                className={`pointer-events-auto max-h-full max-w-full rounded-[1.5rem] border border-white/10 object-contain shadow-[0_60px_120px_rgba(0,0,0,0.9)] transition-all duration-700 md:rounded-[4rem] ${isClosing
          ...
          <div className="pointer-events-none absolute right-0 bottom-8 left-0 z-[1030] flex flex-col items-center md:bottom-16">
            ...
            <div className="mt-4 flex items-center gap-2 opacity-30 md:mt-8">
              <span className="tracking-luxury text-[7px] font-bold text-white uppercase italic md:text-[10px]">
                The Wedding of {firstName} & {secondName}
              </span>
            </div>
          </div>
        </div>
