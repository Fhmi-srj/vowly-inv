...
const Envelope: React.FC<EnvelopeProps> = ({ onOpen }) => {
  ...
  return (
    <div
      className={`bg-darkBg fixed inset-0 z-[2000] flex items-center justify-center overflow-hidden transition-all duration-1000 ease-in-out ${isExiting ? "pointer-events-none scale-110 opacity-0" : "opacity-100"
        }`}
    >
      ...
      <div
        className={`relative z-10 w-full max-w-2xl transform px-6 text-center transition-all duration-1000 ${isAnimate ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0"
          }`}
      >
